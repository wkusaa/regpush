...
Prefer immutable tags such as `sha-<commit>` over `latest`. The optional `insecure-http` input defaults to `false`; enable it only for a trusted local test registry. The optional `cleanup` input defaults to `true`.
